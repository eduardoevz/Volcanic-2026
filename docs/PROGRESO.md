...
- **Salvedad honesta, aplicada del mismo modo que en fases anteriores — sin emulador Android en este entorno:** la verificación de esta fase se limitó a `npx tsc --noEmit`, `npx eslint .`, `npx jest` (los tres en verde) y revisión directa del SQL de las políticas RLS nuevas. No se pudo hacer un recorrido visual en el emulador (Perfil → Directorio → filtros → tarjeta de derivación → navegación), a diferencia de fases donde sí hubo acceso a un dispositivo/emulador real (Fases 6, 8, 10, 11, 13). Documentado con honestidad en vez de simular un resultado — mismo criterio que el build de release bloqueado en Fase 10 o los créditos de API agotados en Fase 7.
- **Aplicación de las migraciones al proyecto remoto:** no se pudo completar en esta sesión por no contar con las credenciales de conexión a la base (`SUPABASE_SERVICE_ROLE_KEY`/cadena de conexión `pg`, usadas en fases anteriores por conexión directa ya que la CLI de Supabase sigue bloqueada por Application Control de Windows) — los archivos `0013`/`0014` quedan versionados en Git, listos para aplicarse con el mismo mecanismo de siempre en cuanto se disponga de esas credenciales. Queda como pendiente explícito, no simulado.
...
- [x] `health_centers`/`specialists` con RLS activo — verificado por revisión directa del SQL (`public_read using (true)` y `public_read_consented using (consent_to_publish = true)` respectivamente); pendiente confirmarlo también por consulta directa contra el proyecto remoto una vez se apliquen las migraciones con credenciales disponibles
...
- [~] Migraciones no aplicadas al proyecto remoto en esta sesión (sin credenciales de conexión disponibles) — SQL versionado y listo, aplicación real queda pendiente

**Fase 14 completa a nivel de código, con dos salvedades honestas documentadas arriba** (sin emulador para verificación visual, migraciones sin aplicar al remoto por falta de credenciales en este entorno) — mismo criterio de honestidad que todas las fases anteriores. Pendiente: aplicar `0013`/`0014` al proyecto remoto y hacer el recorrido visual en el emulador en cuanto el entorno lo permita; después, continuar con Fase 15 (círculo familiar) según `docs/PLAN_DE_IMPLEMENTACION.md` §29.
